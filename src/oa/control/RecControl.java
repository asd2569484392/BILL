		...
		if (StrUtil.isNotEmpty(NRN) && StrUtil.isNotEmpty(item.getText()) && StrUtil.isNotEmpty(order_code.getText())
			...
			if (Util.isInteger(shipNumber.getText()) && Util.isDouble(NRN.getText())
				...
				try {
					...
					dao.addInventoryTransaction(new Inventorytransaction(order_code.getText(), "REC",
							TimeUtil.getyyyyMMdd(), Integer.parseInt(shipNumber.getText()), item.getText(),
							currentLocation.getText().trim(), destinationLocation.getText().trim()));
					LocationDAO locationDAO = session.getMapper(LocationDAO.class);
					locationDAO.addNRO(item.getText(), Integer.parseInt(shipNumber.getText()));
					session.commit();
					clear();
					Dialog.f_alert_informationDialog("收货成功", "已增加库存生成事务");
				...
				Dialog.f_alert_ErrorDialog("错误", "请填写正确物料号");
			}
		} else {
